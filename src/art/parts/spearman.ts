...
/** 手臂（上臂+肘，挂点=肩关节，向下延伸 15px） */
function drawArm(ctx: CanvasRenderingContext2D): void {
  const grad = linearGradient(ctx, -4, 0, 4, 15, [
    [0, '#4d6f9e'],
    [1, '#2c4060'],
  ]);
  fillRoundRect(ctx, -3.2, 0, 6.4, 15, 3, { fill: grad, stroke: OUTLINE, lineWidth: 0.9 });
  // 肩垫
...
function drawHand(ctx: CanvasRenderingContext2D): void {
  fillRoundRect(ctx, -2.4, 0, 4.8, 5, 2, { fill: '#38537c', stroke: OUTLINE, lineWidth: 0.8 });
  fillCircle(ctx, 0, 6.5, 2.4, { fill: '#e8c39e', stroke: OUTLINE, lineWidth: 0.6 });
}

/** 腿（大腿+小腿+靴，挂点=髋，向下延伸 22px） */
function drawLeg(ctx: CanvasRenderingContext2D): void {
  const grad = linearGradient(ctx, -4, 0, 4, 22, [
    [0, '#33466a'],
    [1, '#22304a'],
  ]);
  // 大腿+小腿（锥形）
  ...
  // 护膝
  fillEllipse(ctx, 0, 12.5, 3.4, 2.6, { fill: '#5d6b7d', stroke: OUTLINE, lineWidth: 0.6 });
  // 靴子
  fillPoly(
    ctx,
    [
...
export const SPEARMAN_PARTS: PartDef[] = [
  ...
  { name: 'arm_l', w: 12, h: 18, draw: drawArm },
  { name: 'arm_r', w: 12, h: 18, draw: drawArm },
  { name: 'hand_l', w: 10, h: 10, draw: drawHand },
  { name: 'hand_r', w: 10, h: 10, draw: drawHand },
  { name: 'leg_l', w: 12, h: 26, draw: drawLeg },
  { name: 'leg_r', w: 12, h: 26, draw: drawLeg },
