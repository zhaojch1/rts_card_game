...
function authorIdle(): AnimAuthor {
  return {
    ...
    head: { rf: [rf(1, 0.02, 0.5), rf(1, -0.02, 0.5)] },
    weapon: { rf: [rf(1, 0.03, 0.5), rf(1, -0.03, 0.5)] },
  };
}

/** 移动 walk：步伐 + 重心起伏 + 长枪随动 */
function authorWalk(): AnimAuthor {
  return {
    root: { tf: [tf(0.5, 0, 0, 0.5), tf(0.5, 0, 2.5, 0.5)] },
    leg_l: { rf: [rf(0.5, 0.6, 0.5), rf(0.5, -0.6, 0.5)] },
    leg_r: { rf: [rf(0.5, -0.6, 0.5), rf(0.5, 0.6, 0.5)] },
    torso: { rf: [rf(0.5, -0.07, 0.5), rf(0.5, 0.07, 0.5)] },
    head: { rf: [rf(0.5, -0.04, 0.5), rf(0.5, 0.04, 0.5)] },
    arm_l: { rf: [rf(0.5, 0.35, 0.5), rf(0.5, -0.15, 0.5)] },
    arm_r: { rf: [rf(0.5, -0.35, 0.5), rf(0.5, 0.15, 0.5)] },
    weapon: { rf: [rf(0.5, 0.08, 0.5), rf(0.5, -0.08, 0.5)] },
  };
}

/** 攻击 attack：前摇（前踏蓄力）→ 命中（出枪刺击）→ 收招（收枪回位），命中帧事件 */
function authorAttack(): AnimAuthor {
  return {
    root: { tf: [tf(0.22, -3, 0), tf(0.23, 13, 0), tf(0.25, 0, 0)] },
    torso: { rf: [rf(0.22, 0.16), rf(0.23, -0.2), rf(0.25, 0, 0.5)] },
    head: { rf: [rf(0.22, 0.1), rf(0.23, -0.12), rf(0.25, 0, 0.5)] },
    weapon: { rf: [rf(0.22, -0.9), rf(0.23, -0.05), rf(0.25, -0.2, 0.5)] },
    arm_r: { rf: [rf(0.22, 0.5), rf(0.23, 0.1), rf(0.25, -0.15, 0.5)] },
    arm_l: { rf: [rf(0.22, 0.3), rf(0.23, 0), rf(0.25, 0.15, 0.5)] },
  };
}
...
function authorHit(): AnimAuthor {
  return {
    root: { tf: [tf(0.15, -4, 0), tf(0.25, 0, 0, 0.5)] },
    torso: { rf: [rf(0.15, 0.32), rf(0.25, 0, 0.5)] },
    head: { rf: [rf(0.15, 0.18), rf(0.25, 0, 0.5)] },
    weapon: { rf: [rf(0.15, 0.2), rf(0.25, -0.2, 0.5)] },
    arm_l: { rf: [rf(0.15, 0.3), rf(0.25, 0.15, 0.5)] },
    arm_r: { rf: [rf(0.15, -0.4), rf(0.25, -0.15, 0.5)] },
  };
}

/** 格挡 block：举枪横格（枪体受力偏移留到阶段 3 与命中联动） */
function authorBlock(): AnimAuthor {
  return {
    weapon: { rf: [rf(0.1, -1.25), rf(0.3, -1.25), rf(0.1, -0.2, 0.5)] },
    torso: { rf: [rf(0.1, 0.12), rf(0.3, 0.12), rf(0.1, 0, 0.5)] },
    arm_r: { rf: [rf(0.1, 0.5), rf(0.3, 0.5), rf(0.1, -0.15, 0.5)] },
    arm_l: { rf: [rf(0.1, -0.3), rf(0.3, -0.3), rf(0.1, 0.15, 0.5)] },
    head: { rf: [rf(0.1, 0.08), rf(0.3, 0.08), rf(0.1, 0, 0.5)] },
  };
}

/** 死亡 death：前扑倒地（绕脚底旋转）+ 溶解由渲染层完成 */
function authorDeath(): AnimAuthor {
  return {
    root: { rf: [rf(0.9, 0.5), rf(0.5, 1.45), rf(0.1, 1.45, 0.5)], tf: [tf(0.9, 0, 2), tf(0.5, 0, 6), tf(0.1, 0, 6)] },
    torso: { rf: [rf(0.9, 0.05), rf(0.5, 0.1), rf(0.1, 0.1)] },
    weapon: { rf: [rf(0.9, -0.4), rf(0.5, -0.3), rf(0.1, -0.3)] },
    arm_l: { rf: [rf(0.9, 0.5), rf(0.5, 0.8), rf(0.1, 0.8)] },
    arm_r: { rf: [rf(0.9, -0.6), rf(0.5, -0.9), rf(0.1, -0.9)] },
  };
}

/** 转向 turn：朝向由渲染翻转表现，这里做重心微调 */
function authorTurn(): AnimAuthor {
  return {
    root: { tf: [tf(0.1, 0, 0), tf(0.1, 0, 1.2, 0.5)] },
    torso: { rf: [rf(0.1, 0.06), rf(0.1, -0.06, 0.5)] },
  };
}
